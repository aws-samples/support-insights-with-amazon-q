...
Note that the IAM console will report the IAM policy to be invalid if the StackSet deployment is not complete as the Principal will not yet exist. You have to wait for the StackSet to show as ACTIVE before updating the bucket policy.
You can copy the JSON from the `output_bucket_policy.json` file generated by the script in the root directory. The bucket policy should look like:
