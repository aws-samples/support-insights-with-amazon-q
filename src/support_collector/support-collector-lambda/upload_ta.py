...
checks_info_dict = {check['checkId']: {'name': check['name'], 'description': check['description']} for check in checks_info}

def save_to_s3(recommendations_by_account, bucket_name):
    region = session.region_name
    s3 = session.client('s3', region_name=region)
    current_date = datetime.datetime.utcnow().strftime("%Y-%m-%d")  # Using UTC date to standardize the timestamps across regions
    ...
    for account_id, recommendations in recommendations_by_account.items():
        for recommendation in recommendations:
            ...
            if status in ['warning', 'error', 'yellow', 'red']:  # Filter for warning or error status
                check_id = recommendation['recommendation']['checkId']  # Extract the checkId from the recommendation
                description = checks_info_dict.get(check_id, {}).get('description', 'No description provided')  # Get the description from the checks_info_dict
                # Update the recommendation with name and modified description
                recommendation['recommendation']['description'] = f"The Trusted Advisor (TA) recommendation is for AWS account Id {account_id} that has TA status as '{status}'. This status {status} indicates the account owner should take action on the resources stated here as per this recommendation. The recommendation is as follows: {description}"
                recommendation_json = json.dumps(recommendation, ensure_ascii=False).encode('utf-8')
                file_key = f"ta/{account_id}/{current_date}/{check_id}.json"  # Construct the file key using account_id, date, and checkId
                s3.put_object(
                    Bucket=bucket_name,
                    Key=file_key,
                    Body=recommendation_json
                )
