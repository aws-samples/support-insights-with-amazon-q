...
def save_to_s3(events_by_account, bucket_name):
    ...
    current_date = datetime.datetime.utcnow().strftime("%Y-%m-%d")  # Format the date as YYYY-MM-DD
    ...
    for account_id, account_events in events_by_account.items():
        for event_dict in account_events:
            ...
            arn = event['arn'].split(':')[-1].replace('/', '_')  # Clean ARN for use as filename
            event_json = json.dumps(event, cls=DatetimeEncoder, ensure_ascii=False).encode('utf-8')
...
def list_health_events(past_no_of_days):
    ...
    events_pages = events_paginator.paginate(filter={
        'startTimes': [
            {'from': datetime.datetime.now() - datetime.timedelta(days=past_no_of_days)},
        ...
        for detail in details_response['successfulSet']:
            ...
            for event in health_events:
                if event['arn'] == detail['event']['arn']:
                    event.update({'details': detail['eventDescription']['latestDescription']})
