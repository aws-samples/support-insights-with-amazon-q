...
def lambda_handler(event, context):
    ...
    if run_case:
        ...
        bulk_upload_cases.upload_all_cases_to_s3(bucket_name, past_no_of_days, account_id)
    ...
    if run_ta:
        ...
        response_messages.append('Trusted Advisor recommendations uploaded successfully.')

    if run_health:
        bulk_upload_health = importlib.import_module("upload_health")
        response_messages.append('Searching AWS Health notifications..')
        bulk_upload_health.upload_health_events_to_s3(bucket_name, past_no_of_days, account_id)
